...
public class ChampexamenApplication extends Application {
    ...
    //Added logic clear button
    private void clearExamAnswers() {
        exam.getSubmittedAnswers().clear();
        for (VBox qBox : questionVBoxes) {
            qBox.getChildren().filtered(n -> n instanceof HBox || n instanceof VBox).forEach(node -> {
                if (node instanceof HBox hBox) {
                    hBox.getChildren().forEach(child -> {
                        if (child instanceof RadioButton rb) rb.setSelected(false);
                        if (child instanceof CheckBox cb) cb.setSelected(false);
                    });
        ...
        labelShowingGrade.setText("Grade: Not submitted yet");
    }
